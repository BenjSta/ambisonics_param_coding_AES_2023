...
if __name__ == '__main__':
    ...
    soundfile.write('source_audio/ebu_sqam/50_cut.wav', s, FS)

    s, fs_sig = soundfile.read(os.path.join(my_tmpdir, '49.flac'))
    assert fs_sig == FS
    s = s[start_time_english:end_time_english, 0]
    soundfile.write('source_audio/ebu_sqam/49_cut.wav', s, FS)
    ...
    soundfile.write('source_audio/ebu_sqam/52_cut.wav', s, FS)

    s, fs_sig = soundfile.read(os.path.join(my_tmpdir, '53.flac'))
    assert fs_sig == FS
    s = s[start_time_german_french:end_time_german_french, 0]
    soundfile.write('source_audio/ebu_sqam/53_cut.wav', s, FS)
